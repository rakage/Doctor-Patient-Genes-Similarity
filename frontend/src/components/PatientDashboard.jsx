...
function PatientDashboard() {
  ...
  const fetchGenes = async () => {
    try {
      const response = await axios.get('http://localhost:8000/api/genes/my_genes/', {
        headers: {
  ...
  const handleSaveGene = async (geneData) => {
    try {
      await axios.put(`http://localhost:8000/api/genes/${geneData.id}/`, geneData, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
        }
  ...
  const handleUploadGene = async (formData) => {
    try {
      await axios.post('http://localhost:8000/api/genes/upload/', formData, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
  ...
  const handleDeleteGene = async () => {
    try {
      await axios.delete(`http://localhost:8000/api/genes/delete_gene/`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
          'Content-Type': 'application/json',
        },
  ...
  const handleSaveFormula = async (newFormula) => {
    try {
      await axios.post('http://localhost:8000/api/formulas/', {
        formula: newFormula,
      }, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`,
