...
function ViewSimilarityGeneModal({ geneId, onSave, onClose, show, token }) {
  ...
  useEffect(() => {
    if (show && geneId) {
      ...
      // Fetch gene data
      fetch(`http://localhost:8000/api/genes/${geneId}/`, {
        headers: {
  ...
  const fetchSimilarityResult = async (id) => {
    const token = localStorage.getItem('accessToken');
    try {
      const response = await axios.post('http://localhost:8000/api/genes/calculate_similarity/', {
        patient_gene_id: id,
      }, {
        headers: {
          Authorization: `Bearer ${token}`,
            ...
            {similarityResults.map((result, index) => (
              <div key={index} className='similarity-result'>
                ...
                <div className='similarity-result-content'>
                  ...
                  <div><strong>Gene Sequence:</strong> {result.gene_sequence}</div>
